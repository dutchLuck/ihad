...
ihad [options] [inputFile1 [inputFile2 [.. inputFileN]]]
  where [options] are; -
   ...
   -f N .. Set hex field separator to N character spaces (where 0 <= N <= 2)
   ...
   -L N .. Limit dumps to a maximum of N lines (where N > 0)
   ...
   -v[N] .. Verbose output enable, optionally set level to N (where 0 <= N <= 6)
   -w N .. Set bytes processed per line to N (where 0 < N <= 32)

  where  [inputFile1 [inputFile2 [.. inputFileN]]]
    are optional file name(s) of file(s) to dump. If no input files are specified
    or the file name specified is '-' then input is taken from stdin.

Note that if ihad output isn't to your liking you can try; -
  od -A x -t x1z -v inputFile  or  xxd -g 0 inputFile  or  hexdump -C inputFile
```
